...
// Confirmed protocol: the CLIENT emits 'start-game' with { gameCode } to start a
// game (non-host can do this — it skips the ready-gate). The server then BROADCASTS
// 'game-started' (server-originated) which our phase detection picks up. Earlier we
// wrongly emitted 'game-started' itself, which is broadcast-only and does nothing.
...
// EXPERIMENT — confirmed working: the backend does NOT validate ownership on
// player-joining, so re-emitting another player's join with an override name
// renames them for everyone. We re-assert on both their join AND score updates so
// the rename sticks, and we carry their *current* score/ready (from the roster)
// so spoofing never resets their standings. Loop-safe: once the broadcast already
// shows our override name we stop. Only the primary bot per game emits, to avoid
// N duplicate re-emits when several bots see the same event.
function maybeSpoofNames(session, eventName, args) {
  if (eventName !== 'player-joining' && eventName !== 'update-score') return;
  ...
  if (inGame[0] !== session) return; // primary only
  const ourIds = new Set(inGame.map((s) => s.userId));

  for (const p of collectPlayers(args).values()) {
    if (ourIds.has(p.id) || p.name === spoof.name) continue; // skip ours / already renamed
...
app.post('/api/stax/start', (req, res) => {
  ...
  const payload = { gameCode: session.gameCode };
