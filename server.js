...
function collectPlayerIds(value, ids = new Set()) {
  ...
  if (Array.isArray(value)) {
    for (const item of value) collectPlayerIds(item, ids);
    return ids;
  }

  for (const item of Object.values(value)) collectPlayerIds(item, ids);
  return ids;
}
...
function readyAllPlayers(gameCode) {
  ...
  if (!sockets.length || !playerIds.size) return 0;

  const primarySocket = sockets[0];
  for (const id of playerIds) primarySocket.emit('player-ready', { id });
...
app.post('/api/stax/join', async (req, res) => {
  ...
  socket.on('disconnect', () => {
    ...
    if (![...remoteSessions.values()].some((s) => s.gameCode === gameCode)) {
