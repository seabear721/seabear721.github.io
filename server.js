...
// Pause/resume. 'paused-game' is a confirmed INBOUND event (the game telling us
// it paused). The OUTBOUND emit a client sends to request a pause/resume is NOT
// confirmed for this backend — these defaults are best guesses, overridable via
// env once you capture the real event from the Socket Console (Inbound + Full
// payloads, have a host pause). Detection below tracks the live paused state from
// whatever inbound events actually arrive, so the toggle stays honest regardless.
const PAUSE_EMIT_EVENT = process.env.STAX_PAUSE_EVENT || 'paused-game';
const RESUME_EMIT_EVENT = process.env.STAX_RESUME_EVENT || 'resume-game';

function isGamePauseEvent(name) {
  return /paus/i.test(name) && !/unpaus|resume/i.test(name);
}
function isGameResumeEvent(name) {
  return /resume|unpaus/i.test(name);
}
...
function updateGamePhase(gameCode, eventName) {
  ...
  if (!phase) {
    phase = { started: false, startedAt: null, paused: false, lastSignal: null, updatedAt: now() };
  ...
  } else if (phase.started && !phase.paused && isGamePauseEvent(eventName)) {
    phase.paused = true;
    phase.lastSignal = eventName;
    phase.updatedAt = now();
    addDebugLog('game-phase', { gameCode, phase: 'paused', eventName });
  } else if (phase.started && phase.paused && isGameResumeEvent(eventName)) {
    phase.paused = false;
    phase.lastSignal = eventName;
    phase.updatedAt = now();
    addDebugLog('game-phase', { gameCode, phase: 'resumed', eventName });
  }
}
...
app.get('/api/stax/roster', (req, res) => {
  ...
  // an all-games query, started/paused are true if any game our bots are in is.
  const started = gameCode
    ? !!gamePhases.get(gameCode)?.started
    : [...remoteSessions.values()].some((s) => gamePhases.get(s.gameCode)?.started);
  const paused = gameCode
    ? !!gamePhases.get(gameCode)?.paused
    : [...remoteSessions.values()].some((s) => gamePhases.get(s.gameCode)?.paused);

  res.json({ ok: true, gameCode, started, paused, count: players.length, players });
});
...
// Pause / resume the game. Only valid once the game has started. Emits a
// best-guess event (PAUSE_EMIT_EVENT / RESUME_EMIT_EVENT, env-overridable) from
// one connected bot in the game. The paused state shown to the UI is driven by
// inbound events, but we set it optimistically here for immediate feedback.
app.post('/api/stax/pause', (req, res) => {
  const gameCode = req.body?.gameCode ? cleanGameCode(req.body.gameCode) : null;
  const wantPaused = req.body?.paused;
  if (typeof wantPaused !== 'boolean') return res.status(400).json({ error: 'paused must be a boolean' });
  ...
  if (!session) {
    ...
    return res.status(409).json({ error: 'The game has not started yet — pause only works mid-game.' });
  }

  const eventName = wantPaused ? PAUSE_EMIT_EVENT : RESUME_EMIT_EVENT;
  const payload = { game: session.gameId };
  logOutbound(session, eventName, payload);
  session.socket.emit(eventName, payload);

  const phase = gamePhases.get(session.gameCode);
  if (phase) {
    phase.paused = wantPaused; // optimistic; inbound events keep this honest
    phase.updatedAt = now();
  }

  res.json({ ok: true, gameCode: session.gameCode, paused: wantPaused, event: eventName });
});
