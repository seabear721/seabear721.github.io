...
const nameSpoof = new Map(); // gameCode -> { enabled, name, attempted:Set<id> }
const debugLogs = [];
const debugSettings = {
  socketEvents: false,
  joinAck: false,
...
// Best-guess OUTBOUND start event. The game broadcasts 'game-started' bare on
// start (confirmed from a live capture); whether a non-host emitting it actually
// starts the game is what we're testing. Env-overridable.
const START_EMIT_EVENT = process.env.STAX_START_EVENT || 'game-started';
...
// EXPERIMENT — when name-spoof is on for a game, try to rename *other* players by
// re-emitting their join with an override name, to test whether the backend lets
// a non-host rename a player and have it sync. The attempted-set prevents loops
// (we only try each id once per toggle-on, and skip players already renamed).
function maybeSpoofNames(session, eventName, args) {
  if (eventName !== 'player-joining') return;
  const spoof = nameSpoof.get(session.gameCode);
  if (!spoof?.enabled || !spoof.name) return;
  const ourIds = new Set(
    [...remoteSessions.values()].filter((s) => s.gameCode === session.gameCode).map((s) => s.userId)
  );
  for (const p of collectPlayers(args).values()) {
    if (ourIds.has(p.id) || p.name === spoof.name || spoof.attempted.has(p.id)) continue;
    spoof.attempted.add(p.id);
    const renamed = { id: p.id, name: spoof.name, socketID: p.socketID, score: p.score ?? 0, ready: p.ready ?? false };
    logOutbound(session, 'player-joining', { reason: 'name-spoof', id: p.id, name: spoof.name });
    session.socket.emit('player-joining', session.gameObj, renamed);
  }
}
...
// EXPERIMENT — Start the game. Only valid in pregame. Emits a best-guess start
// event (START_EMIT_EVENT, env-overridable) bare from one connected pregame bot.
// Whether a non-host can actually start the game is what this tests; watch the
// Socket Console for a 'game-started' echo to know if the backend accepted it.
app.post('/api/stax/start', (req, res) => {
  ...
  logOutbound(session, START_EMIT_EVENT, {});
  session.socket.emit(START_EMIT_EVENT);
...
app.post('/api/stax/name-spoof', (req, res) => {
  ...
  for (const code of codes) {
    if (enabled) nameSpoof.set(code, { enabled: true, name, attempted: new Set() });
