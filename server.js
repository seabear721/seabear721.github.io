...
// Pause/resume. Confirmed from a live capture (2026-05-31): the game broadcasts
// 'paused-game' / 'unpaused-game' with an empty payload. We emit those bare to
// request the state change. Still env-overridable. Detection below tracks the
// live paused state from inbound events, so the toggle stays honest regardless.
const PAUSE_EMIT_EVENT = process.env.STAX_PAUSE_EVENT || 'paused-game';
const RESUME_EMIT_EVENT = process.env.STAX_RESUME_EVENT || 'unpaused-game';
...
app.post('/api/stax/pause', (req, res) => {
  ...
  // Live capture shows these events carry no payload, so emit them bare to match.
  const eventName = wantPaused ? PAUSE_EMIT_EVENT : RESUME_EMIT_EVENT;
  logOutbound(session, eventName, {});
